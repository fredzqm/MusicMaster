...
/*
 *	"lcd8bits.c" LCD Display Panel Driver Routines
...
#define LCD_RS                  RD2         //rename RC2 	-LCD command/data 
#define LCD_EN                  RD3         //rename RC3	-LCD Enable (clocks data into LCD on falling edge) 
...
#define LCDCMD_FunctionSet      0x28    //function set: 8-bit mode, 2 lines, 5x7 dots
...
void lcd_write(unsigned char mode, unsigned char CmdChar) {
    char lower = (CmdChar << 4) & 0xf0 ;
    char higher = CmdChar & 0xf0 ;
    LCD_DATA = LCD_DATA & 0x0f;
    LCD_DATA = LCD_DATA | higher;
    LCD_RS = mode;
    LCD_strobe(); // Write 8 bits of data on D7-0
    LCD_DATA = LCD_DATA & 0x0f;
    LCD_DATA = LCD_DATA | lower;
    LCD_RS = mode;
    LCD_strobe(); // Write 8 bits of data on D7-0
}
